...
CLI commands to run from the APIC or switches to glean information around the configuration and operational state of the fabric. Quicker than the GUI in most cases and useful for troubleshooting and planning. This article is far from comprehensive, but covers some of the commands I find most useful...
...
moquery -c infraAccBndlGrp | grep PG | sed -n 's/.*acc\(portgrp\|bundle\)-\(.*\)/\2/p' | sort | uniq >> pg-configured
